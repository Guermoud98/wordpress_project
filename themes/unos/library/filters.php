...
function hoot_archive_title_filter( $title ) {
	...
	elseif ( is_category() )
		$title = str_replace( __( 'Category: ', 'unos' ), '', $title );

	elseif ( is_tag() )
		$title = str_replace( __( 'Tag: ', 'unos' ), '', $title );

	elseif ( is_tax() && !is_tax( 'post_format' ) )
		$title = single_term_title( '', false );

	elseif ( is_author() )
		$title = str_replace( __( 'Author: ', 'unos' ), '', $title );
	...
	elseif ( is_post_type_archive() )
		$title = str_replace( __( 'Archives: ', 'unos' ), '', $title );
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$suffix = ' <span class="loop-title-suffix">' . sprintf( __( '(Page %s)', 'unos' ), $paged ) . '</span>';
